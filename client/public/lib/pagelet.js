                    ...
                    if ((xhr.status >= 200 && xhr.status < 300) || xhr.status == 304) {
                        ...
                        try {
                            ...
                            if (typeof(report) != 'undefined') {
                                var range = 50;
                                var msg = [e.message, result.substr(0,range), result.substr(result.length-range,result.length)].join('$@$');
                                report({
                                    action : 'page_status',
                                    a1 : window.config && config.path || '----',
                                    a2 : 'pagelet_error',
                                    a3 : encodeURIComponent(msg)
                                });
                            }
                        }
                        if (error) {
                            ...
                            //}
                            if (result.hash && result.hash !== hash) {
                                pagelet.emit(pagelet.EVENT_LOAD_ERROR, { error: 'hash inconsistency' });
                            }
                            var res = [];
