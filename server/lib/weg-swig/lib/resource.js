...
Resource.setRoot = function (rootDir) {
    root = rootDir;
};
 ...
 * @type {string}
 */
Resource.prototype.JS_HOOK = '<!--WEG_JS_HOOK-->';

/**
 * link标签占位
 * @type {string}
 */
Resource.prototype.CSS_HOOK = '<!--WEG_CSS_HOOK-->';

/**
 * 首屏分隔占位
 * @type {string}
 */
Resource.prototype.ATF_HOOK = '<!--WEG_ATF_HOOK-->';
...
Resource.prototype.getResById = function (id) {
    //console.log('>>>>getResById',id, options.res[id]);
    if (options) {
        if (options.res) {
...
Resource.prototype.normalize = function (id, ext) {
    ...
    if(ext=='.html'){
        return path.join(PREFIX , 'views', result);
    }else{
        //console.log('normalize:', id);
        return path.join(PREFIX, result.replace(/^(client)/, ''));
    }
};
...
Resource.prototype.include = function (file, ctx, swig) {
    ...
    if (file.indexOf('.') === -1) {
        ...
        if (res) {
            ...
            var opt = {};
            if (root) {
                opt.resolveFrom = root + '/index.html';
            }
            //console.log('html:', path.join(root, res.uri));
            html = swig.compileFile(path.join(root, res.uri), opt)(ctx);
        }
    }
